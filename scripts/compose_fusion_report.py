#!/usr/bin/env python3
"""Compose a due-diligence report.

Default mode (direct-mcp): connect to 5 MCP servers (enterprise / risk /
operation / patent / recruitment), pull raw data, run cross-domain analysis,
and produce a rich report with cross-dimensional insights + specialty score
matrix + structured verdict. Use ``--dry-run`` to read cached reports_探迹/
...
Usage::
  ...
  python compose_fusion_report.py --enterprise "广州探迹科技有限公司" \\
    --dry-run --output output/尽调.json --report-output output/尽调.html

  # real run (needs MCP connection configured)
  python compose_fusion_report.py --enterprise "广州探迹科技有限公司" \\
    --output output/尽调.json --report-output output/尽调.html

  # legacy: aggregate pre-existing atomic reports
  python compose_fusion_report.py --enterprise "广州探迹科技有限公司" \\
    --reports-dir ../../reports_探迹 --output output/尽调.json --report-output output/尽调.html
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compose a due-diligence report.")
    ...
    print(f"已聚合 {len(reports)}/{len(all_atomic)} 个原子报告 (场景: due_diligence)", file=sys.stderr)
    payload = build_fusion_payload(args.enterprise, reports, source_mode=source_mode, scenario="due_diligence")
    _emit(payload, args)
