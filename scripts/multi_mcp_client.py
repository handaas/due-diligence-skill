...
The due-diligence scenario needs cross-domain data from 5 MCP servers:
  enterprise / enterprise-risk / enterprise-operation / patent / recruitment.
...
SKILL_MCP_TOKEN = "DUE_DILIGENCE_MCP_TOKEN"
...
class MultiMcpClient:
    ...
    async def ensure_session(self, domain: str) -> Any:
        ...
        if not conn:
            raise MultiMcpError(
                f"未配置 MCP 连接 [{domain}]。请设置 DUE_DILIGENCE_MCP_TOKEN 或 {REMOTE_URL_ENV.get(domain, 'HANDAAS_MCP_URL')}（Remote），"
